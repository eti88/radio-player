...
class Api {
  ...
  getExploreCountries() {
    return fetch(`${this.url}/explore/countries`).then(response => response.json())
  }

  getExploreGenres() {
    return fetch(`${this.url}/explore/genres`).then(response => response.json())
  }
  ...
  getRadioStreamBySlug(slug) {
    return fetch(`${this.url}/radio/${slug}/stream`).then(response => response.json())
  }
